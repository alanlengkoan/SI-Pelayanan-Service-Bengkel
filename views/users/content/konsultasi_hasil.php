...
<section role="main" class="content-body">
    ...
    foreach ($data as $keyData => $valueData) {
        ...
        foreach ($responden as $keyResponden => $valueResponden) {
            if ($valueData['id_responden'] === $keyResponden) {
                ...
                $kelasId[$valueResponden] = $valueData['id_responden'];
            }
        }
    }
    ...
    // ambil lokasi yg akan dijadikan konsultasi
    $id  = $_POST['inpidlahan'];
    $qry = $pdo->GetWhere('tb_lahan', 'id_lokasi', $id);

    while ($row = $qry->fetch(PDO::FETCH_OBJ)) {
        $konsultasi[$row->id_atribut] = $row->nilai;
    }
                ...
                <table class="table table-bordered table-striped table-condensed mb-none">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($kelas as $keyKelas => $valueKelas) {
                            ...
                            $lastResult[$kelasId[$keyKelas]] = $hitung;
                        ?>
                            <td><?= $hitung ?></td>
                        <?php } ?>
                        <td>
    ...
    <?php
    // untuk insert data ke laporan
    $maxResult     = max($lastResult);
    $maxResultKeys = array_keys($lastResult, $maxResult);
    $id_responden  = $maxResultKeys[0];
    $id_users      = $rowLog->id_users;

    $pdo->Insert("tb_laporan", ["id_users", "id_lokasi", "id_responden"], [$id_users, $id, $id_responden]);
    ?>
    <!-- end:: tabel -->
</section>